...
- **渲染截图** — 支持 front / top / side_45 三种预设角度的高质量截图，自动计算相机距离确保留白 5%，表单颜色和材质自动从柜子模型中取出现次数最多的板件值
- **渲染图上传** — 截图转为 File 对象通过 `multipart/form-data` 上传至 `/render_api/images/upload`，渲染请求使用 `image_id` 标识图片
...
└── frontend/                         # Vue3 前端
    ...
    │   ├── components/
    │   │   ├── HeaderBar.vue         # 顶部工具栏（撤销/重做/保存/渲染/工具/主题）
