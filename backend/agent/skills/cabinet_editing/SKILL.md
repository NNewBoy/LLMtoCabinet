...
- **单开门/双开门的 width 是门板厚度（与 thickness 一致，如18mm），不是柜子深度！**
- **门板应放在柜子正面：position.z = 柜子 width（如600），开口方向为 +Z**
...
- 单开门/门板（door）：length=门宽, width=板厚(=thickness, 默认18), height=门高，position.z=柜深，绕左侧边旋转打开
- 双开门（double_door）：length=门宽, width=板厚(=thickness, 默认18), height=门高，position.z=柜深，两块 door（左门和右门）
  - 添加 double_door 时，系统会自动创建两块 door 子组件
  - 左门：position.x=0, length=柜内宽/2
  - 右门：position.x=柜内宽/2, length=柜内宽/2
- 抽屉（drawer）：length=抽面宽, width=柜深-背板厚+板件厚度(如600-5+18=613), height=抽面高，position.z=背板厚度，伸出柜子前面18mm
...
```
double_door (父组件)
├── left_door (door, children[0])
│   └── handle (可选拉手)
└── right_door (door, children[1])
    └── handle (可选拉手)
```
- 父组件 double_door 的 dimensions 表示整个门板区域
- 子组件 door 的 position 是相对于父组件的坐标
...
## 常见场景（附完整参数示例，假设柜子 800x600x2000，侧板厚18mm，背板厚5mm）
- 注意：单开门(door)和双开门(double_door)的 width 是门板厚度（与 thickness 一致，如18mm），**不是柜子深度**。请勿将 width 设为柜子 width（如600）！
- 门板/单开门/双开门应放在柜子正面：position.z = 柜子 width（如 600），开口方向为 +Z。拉手作为门/抽屉子组件时也放在父组件 +Z 正面外侧。
- 抽屉 width = 柜深 - 背板厚 + 板件厚度（如 600-5+18=613），position.z = 背板厚度（如5），伸出柜子前面18mm
...
- "加一扇单开门" → add_component(type="door", name="左门板", position={"x":18,"y":18,"z":600}, dimensions={"length":764,"width":18,"height":1964})
  - 系统会自动创建单开门及竖拉手
- "加双开门" → add_component(type="double_door", name="双开门", position={"x":18,"y":18,"z":600}, dimensions={"length":764,"width":18,"height":1964})
  - 系统会自动创建双开门及左右门拉手
- "加一个抽屉" → add_component(type="drawer", name="抽屉", position={"x":18,"y":500,"z":5}, dimensions={"length":764,"width":613,"height":200})
  - 系统会自动创建抽屉及横拉手
